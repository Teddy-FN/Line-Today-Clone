...
function ArticleMovie1() {
    const [article, setArticle] = useState([])

    useEffect(() => {
        axios.get('/id/portaljson')
            .then((res) => {
                console.log('MOVIE SECTIONS', res.data.result.categories[11].templates[1].sections[0].articles)
                setArticle(res.data.result.categories[11].templates[1].sections[0].articles)
            })
    }, [])
