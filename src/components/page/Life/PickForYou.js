...
function TrendingLife() {
    const [article, setArticle] = useState([])

    useEffect(() => {
        axios.get('/id/portaljson')
            .then((res) => {
                console.log('ENGLISH SECTIONS', res.data.result.categories[3].templates[15].sections[0].articles)
                setArticle(res.data.result.categories[3].templates[15].sections[0].articles)
            })
    }, [])
